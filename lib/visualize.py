...
def save_individual_images(ori_img,pred_res,gt,filename):
    ori_img = data = np.transpose(ori_img,(1,2,0))
    pred_res = data = np.transpose(pred_res,(1,2,0))
    gt = data = np.transpose(gt,(1,2,0))
    ...
    if ori_img.shape[2]==3:
        ...
        binary = np.repeat((binary*255).astype(np.uint8),repeats=3,axis=2)
        gt = np.repeat((gt*255).astype(np.uint8),repeats=3,axis=2)
        save_img(pred_res, join('/home/tgada/Unet/VesselSeg-Pytorch/experiments/UNet_vessel_seg/result_img/pred_res',filename))
        save_img(binary, join('/home/tgada/Unet/VesselSeg-Pytorch/experiments/UNet_vessel_seg/result_img/binary',filename))
        save_img(gt, join('/home/tgada/Unet/VesselSeg-Pytorch/experiments/UNet_vessel_seg/result_img/gt',filename))
        save_img(ori_img, join('/home/tgada/Unet/VesselSeg-Pytorch/experiments/UNet_vessel_seg/result_img/ori_img',filename))
    return None
