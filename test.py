...
from lib.visualize import save_img,group_images,concat_result,save_individual_images
import os
import argparse
...
class Test():
    ...
    def save_segmentation_result(self):
        img_path_list = load_file_path_txt(self.args.test_data_path_list)
        img_name_list = [item.split('/')[-1].split('.')[0] for item in img_path_list]

        kill_border(self.pred_imgs, self.test_masks) # only for visualization
        ...
        for i in range(self.test_imgs.shape[0]):
            ...
            save_individual_images(self.test_imgs[i],self.pred_imgs[i],self.test_masks[i],"Individual_"+img_name_list[i]+'.png') 
...
#         log = OrderedDict([('val_auc_roc', eval.auc_roc()),
        log = OrderedDict([('val_f1', eval.f1_score()),
                           ('val_acc', accuracy),
                           ('SE', sensitivity),
                           ('SP', specificity)])
        return dict_round(log, 6)
